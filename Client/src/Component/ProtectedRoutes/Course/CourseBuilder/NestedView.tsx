...
import { useDeleteSectionMutation, useDeleteSubSectionMutation } from "../../../../Services/Operation/authApi"

export default function NestedView({ handleChangeEditSectionName } : any) {
...
const [DeleteSection] = useDeleteSectionMutation();
  ...
  const handleDeleleSection = async (sectionId : any) => {
    const result = await DeleteSection({
      sectionId,
      courseId: course._id
    }).unwrap();
    // console.log("deletesecton",result)
    if (result) {
  ...
  const handleDeleteSubSection = async (subSectionId : any, sectionId : any) => {
    const result = await DeleteSubSection({ subSectionId, sectionId})
    // console.log("deleted subsection ",result.data.data)
    if (result) {
      // update the structure of course
      const updatedCourseContent = course.courseContent.map((section : any) =>
        section._id === sectionId ? result.data.data : section
      )
